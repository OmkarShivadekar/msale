				...
				if(status==1)
				{
					if(request.getParameter("UName").toString().trim()!=null && request.getParameter("UPass").toString().trim()!=null)
					{
						...
						if(userStatus==1)
						{
							...
							request.setAttribute("status", "User Created Successfully!.Now Create User Invoice");
							RequestDispatcher rd= request.getRequestDispatcher("jsp/sadmin/CreateInvoice.jsp");
